...
data MyState
  = MyState {editor :: Editor Text (), document :: Document RgaString}
...
sync storage state@MyState {editor, document} reload =
  runStorage storage $ do
    do
      let Document {objectFrame} = document
      objectFrame' <-
...
replaceZipper text zipper = moveCursorClosely originalPosition zipper'
  where
