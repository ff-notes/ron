...
import RON.Schema
import RON.Schema.EDN qualified as EDN
import RON.Schema.TH.Common (mkGuideType, mkNameT)
import RON.Schema.TH.Struct (mkReplicatedStructLww, mkReplicatedStructSet)
...
mkReplicated' = fmap fold . traverse fromDecl
  where
...
mkEnum Enum{name, items} = do
  ...
  where
    ...
    mkInstanceReplicatedAsPayload itemsUuids =
        [d|
