...
prelude =
    Map.fromList
        [
            ( "Bool"
            , Type0 $
                opaqueAtoms "Bool" OpaqueAnnotations{haskellType = Just "Bool"}
            )
...
data StructConfig = StructConfig
    ...
    , parseMergeStrategy ::
        [TaggedValue] ->
        Parser (MergeStrategy, [TaggedValue])
    , errorMessage :: String
    }
  ...
  where
    validateName name =
        unless (name `Map.member` userTypes || name `Map.member` prelude) $
            fail $
                "validateParsed: unknown type name " ++ Text.unpack name
    validateExpr = \case
        ...
        validateField fieldName field =
          ...
          where
            ...
            fail'' msg =
                fail' $
                    msg
                        <> ", valid merge strategies for objects are:\
                           \ monoid, set"
...
evalSchema env = fst <$> userTypes'
  where
    ...
    evalStruct Struct{..} =
        Struct{fields = evalField <$> fields, ..}

    evalField :: Field Parsed -> Field Resolved
    evalField Field{..} =
        Field{ronType = evalType ronType, ..}
